...
TEST(ThreeAdressCode, Generate_IF)
{
    ...
    auto tac = mCc_ast_get_tac_statement(result.program->function_def_list->function_def->compound_stmt->statement_list->statement);
    mCc_tac_print_tac(tac, stderr);
...
TEST(ThreeAdressCode, Generate_IFELSE)
{
    ...
    ASSERT_EQ(TAC_LINE_TYPE_LABEL,tac->next->next->next->next->next->next->type);
    ...
    auto labelELSE=tac->next->next->next->next->next->next->type_label.label_name;
...
TEST(ThreeAdressCode, Generate_WHILE)
{
    ...
    auto endLabel = tac->next->next->next->next->next->next->type_label.label_name;
...
{
    const char input[] =
            "int foo(){return 1;} int main(){int a; a=foo();}";
    auto result = mCc_parser_parse_string(input);
...
TEST(ThreeAdressCode, FUNCTION_CALL_WITH_PARAMS)
{
    ...
    ASSERT_EQ(TAC_LINE_TYPE_PUSH,tac->next->next->next->next->next->next->next->next->type);
    ASSERT_EQ(TAC_LINE_TYPE_PUSH,tac->next->next->next->next->next->next->next->next->next->next->next->type);

    auto pushI=tac->next->next->next->next->next->next->next->next->type_push.var;
    auto pushS=tac->next->next->next->next->next->next->next->next->next->next->next->type_push.var;
...
{
    const char input[] =
            "void fun(){int[5] arr; int a; a=arr[4];}";
    auto result = mCc_parser_parse_string(input);
    if (result.parse_error.is_error) {
        print_error(result.parse_error);
    }
    ...
    ASSERT_EQ(line6->type_assign_array.arr.type, line6->type_assign_array.var.type);
