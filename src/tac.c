    ...
    while (p != NULL) {
        switch (p->type) {
            ...
            case TAC_LINE_TYPE_DOUBLE:
                ...
                break;
            default: fprintf(out, "%s", p->arg0); break;
        }
        p = p->next;
    }
}
...
void mCc_tac_emitSpecial(mCc_tac_node tac, char *str, enum mCc_tac_line_type type)
{
    mCc_tac_add_node(tac, str, NULL, NULL, NULL, type);
}

void mCc_tac_emitS(mCc_tac_node tac, enum mCc_tac_line_type type, char *format,...)
{
    ...
    va_end(argptr);
    mCc_tac_emitSpecial(tac, str, type);
}
    ...
    switch (expression->type) {
        ...
        case MCC_AST_EXPRESSION_TYPE_CALL_EXPR: {
            ...
            while (next != NULL) {
                ...
                mCc_tac_emitS(tac,TAC_LINE_TYPE_PUSH,"PushStack(%s)\n", mCc_tac_get_tac_var(t));
                next = next->next;
            }
            ...
            mCc_tac_emitS(tac, TAC_LINE_TYPE_CALL,"Call (%s)\n",
                          expression->call_expr.identifier->name);
            mCc_tac_emitS(tac, TAC_LINE_TYPE_POP,"%s = PopStack()\n", mCc_tac_get_tac_var(ret));
            break;
        }
                            ...
                            mCc_tac_node tac)
{
    switch (statement->type) {
        case MCC_AST_STATEMENT_TYPE_WHILE: {
            ...
            mCc_tac_emitS(tac, TAC_LINE_TYPE_LABEL,"L%d:\n", labelBefore);
            struct mCc_tac_var t =
                    mCc_tac_cgen_expression(statement->while_condition, tac);
            mCc_tac_emitS(tac, TAC_LINE_TYPE_IFZ,"Ifz %s Goto L%d\n", mCc_tac_get_tac_var(t),
                          labelAfter);
            mCc_tac_cgen_statement(statement->body, tac);
            mCc_tac_emitS(tac, TAC_LINE_TYPE_JUMP, "Goto L%d\n", labelBefore);
            mCc_tac_emitS(tac, TAC_LINE_TYPE_LABEL, "L%d:\n", labelAfter);
            break;
        }
        ...
        case MCC_AST_STATEMENT_TYPE_RETURN: {
            ...
            mCc_tac_emitS(tac, TAC_LINE_TYPE_RETURN,"RETURN %s\n", mCc_tac_get_tac_var(label));
            break;
        }
        case MCC_AST_STATEMENT_TYPE_DECLARATION:
            switch (statement->declaration->declaration_type) {
                case MCC_AST_DECLARATION_TYPE_ARRAY_DECLARATION: {
                    mCc_tac_emitS(
                            tac, TAC_LINE_TYPE_DECL_ARRAY, "ARRAY_DECL %s %s (%ld) \n",
        ...
        case MCC_AST_STATEMENT_TYPE_ARRAY_ASSIGNMENT: {
            ...
            mCc_tac_emitS(tac, TAC_LINE_TYPE_ASSIGNMENT_ARRAY, "ARRAY_ASSIGNMENT %s[%s]=%s\n",
...
void mCc_tac_cgen_function_def(struct mCc_ast_function_def *function_def,
                               mCc_tac_node tac)
{
    mCc_tac_emitS(tac, TAC_LINE_TYPE_LABELFUNC,"START FUNC %s\n",
    ...
    while (next != NULL) {
        switch (next->declaration->declaration_type) {
            case MCC_AST_DECLARATION_TYPE_DECLARATION: {
                mCc_tac_emitS(
                        tac, TAC_LINE_TYPE_POP,"%s = PopStack(%s)\n",
            ...
            case MCC_AST_DECLARATION_TYPE_ARRAY_DECLARATION: {
                mCc_tac_emitS(
                        tac,TAC_LINE_TYPE_POPARRAY, "%s = PopStackArray(%s_%ld)\n",
    ...
    mCc_tac_emitS(tac,TAC_LINE_TYPE_LABELFUNC_END,"END FUNC %s\n",
                  function_def->function_identifier->name);
}
