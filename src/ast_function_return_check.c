...
void mCc_ast_check_function_void_compound(
                            ...
                            struct mCc_err_error_manager *error_manager)
{
	if (function_def->return_type ==
	    MCC_AST_TYPE_VOID) { // check for absence of return statement
		// if there is no compound statement there can be no return statement
		if (function_def->compound_stmt != NULL) {
			mCc_ast_check_function_void_compound(function_def->compound_stmt,
			                                     error_manager);
		}
